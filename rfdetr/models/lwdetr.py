...
from rfdetr.models.backbone.projector import build_multiscale_projector

from rfdetr.util import box_ops
from rfdetr.util.misc import (accuracy, get_world_size, is_dist_avail_and_initialized)

from rfdetr.models.backbone import build_backbone
from rfdetr.models.matcher import build_matcher
from rfdetr.models.transformer import build_transformer
from rfdetr.models.decoder import TransformerDecoder, TransformerDecoderLayer
from rfdetr.models.decoder import Decoder
...
class LWDETR(nn.Module):
    def __init__(self,
                 ...
                 bbox_reparam=False, position_embedding=None, projector=None):
        super().__init__()
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        if not features:
            raise ValueError(" Backbone returned empty feature list — check input image dimensions or dataset pipeline.")
        ...
        for feat in features:
            if isinstance(feat, tuple):
                ...
                src = feat
                mask = samples.mask if hasattr(samples, 'mask') else torch.zeros_like(src[:, 0, :, :], dtype=torch.bool)
            srcs.append(src)
            masks.append(mask)
            assert mask is not None

        if self.position_embedding is not None and isinstance(self.position_embedding, torch.nn.Module):
            pos = []
            for i, (src, mask) in enumerate(zip(srcs, masks)):
                nested = NestedTensor(src, mask)
                pe = self.position_embedding(nested)
            if pe is None:
                raise ValueError(f" position_embedding returned None at level {i}")
            pos.append(pe)
        else:
            raise ValueError(" position_embedding module is not set properly.")
        ...
        hs, ref_unsigmoid, hs_enc, ref_enc = self.transformer(srcs, masks, pos, refpoint_embed_weight, query_feat_weight)
...
"""
    def forward(self, samples: NestedTensor, targets=None):
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features = [f.tensors if isinstance(f, NestedTensor) else f for f in features]
        features = self.projector(features)

        print("🔥 backbone output:", features, type(features))
        if isinstance(features, list):
            for i, f in enumerate(features):
                print(f"Layer {i}: {type(f)} → {f.shape if hasattr(f, 'shape') else f}")


        if poss is not None and isinstance(self.position_embedding, torch.nn.Module):
            pos = self.position_embedding([f.mask for f in features])
        else:
            pos = [None for _ in features]
...
def build_model(args):
    ...
    # ✅ Return both backbone and position_embedding explicitly
    backbone, position_embedding = build_backbone(
        ...
        projector_scale=getattr(args, 'projector_scale', ['P3', 'P4', 'P5']),
    ...
    projector = build_multiscale_projector(args)
    args.num_feature_levels = len(args.projector_scale)

    transformer = build_transformer(args, decoder_class=Decoder)
