...
from rfdetr.models import decoder
import torch
import torch.nn.functional as F
from torch import nn, Tensor
from rfdetr.models.decoder import DecoderLayer as TransformerDecoderLayer

from rfdetr.models.attention import MultiheadAttention
from rfdetr.models.ops.modules import MSDeformAttn

from rfdetr.models.decoder import Decoder as TransformerDecoder
...
class Transformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat):
        ...
        for lvl, (src, pos_embed) in enumerate(zip(srcs, pos_embeds)):
            if pos_embed is None:
                raise ValueError(f" pos_embed is None at level {lvl}")
            bs, c, h, w = src.shape
        ...
        if masks is not None:
            ...
            for lvl, mask in enumerate(masks):
                expected_shape = srcs[lvl].shape[-2:]  # (H, W) of feature map
                m_resized = F.interpolate(mask[None].float(), size=expected_shape, mode="nearest")[0]
                resized_masks.append(m_resized > 0.5)

            mask_flatten = torch.cat([m.flatten(1) for m in resized_masks], 1)
            valid_ratios = torch.stack([self.get_valid_ratio(m) for m in resized_masks], 0).unsqueeze(0).repeat(bs, 1, 1)
    ...
    def get_valid_ratio(self, mask):
        if mask.dim() == 3:
            _, H, W = mask.shape
            valid_H = torch.sum(~mask[0, :, 0])
            valid_W = torch.sum(~mask[0, 0, :])
        elif mask.dim() == 2:
            H, W = mask.shape
            valid_H = torch.sum(~mask[:, 0])
            valid_W = torch.sum(~mask[0, :])
        else:
            raise ValueError(f"Unexpected mask shape: {mask.shape}")

        valid_ratio_h = valid_H.float() / H
        valid_ratio_w = valid_W.float() / W
        return torch.stack([valid_ratio_w, valid_ratio_h], dim=0)
...
def build_transformer(args, decoder_class=None):
    return Transformer(
