...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    if tensor_list[0].ndim == 3:
        ...
        for i, (img, pad_img, m) in enumerate(zip(tensor_list, tensor, mask)):
            if img.numel() == 0:
                raise ValueError(f"Image at index {i} is empty. Check your dataset.")
            pad_img[: img.shape[0], : img.shape[1], : img.shape[2]].copy_(img)
            m[: img.shape[1], :img.shape[2]] = False

        if mask is None or mask.numel() == 0:
            raise ValueError("Generated mask is None or empty in nested_tensor_from_tensor_list.")
    else:
...
def validate_dataset(dataset, limit=20):
    print("\n Validating dataset tensors...")
    for idx in range(min(len(dataset), limit)):
        img, target = dataset[idx]
        if not isinstance(img, torch.Tensor):
            raise TypeError(f"Invalid type at index {idx}: {type(img)}")
        if img.ndim != 3:
            raise ValueError(f"Expected 3D image tensor at index {idx}, got shape {img.shape}")
        if img.numel() == 0:
            raise ValueError(f"Image at index {idx} is empty.")
    print(" Dataset image tensors look valid.\n")
